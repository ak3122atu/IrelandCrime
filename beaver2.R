...
#temp (dependent var) = not normally distributed
#activity (independent var) = normally distributed 

#format of the test is wilcox.test
wilcox.test(dependent ~ independent)
wilcox.test(temp ~ activity)

#p value < 0.05 so the null hypothesis is rejected 
#and it is concluded beaver body temperature is affected 
#by activity (p= 2.2e - 16)